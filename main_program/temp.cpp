//
// Created by supragyaraj on 3/3/18.
//
...
struct message{
    ...
    enum filetype ftype;
} recieved_transaction[100];

bool valid_transaction[100];
        ...
        if(incoming->mtype == TRY_SEND){
            ...
            if(incoming->fctype == CREATE || incoming->fctype == MODIFY){
                ...
                if(incoming->ftype == DIRECTORY){
                    ...
                    mtx.lock();
                    for(unsigned int i=0; i<100; i++){
                        if(!valid_transaction[i]){
                            recieved_transaction[i].fctype = incoming->fctype;
                            recieved_transaction[i].ftype = DIRECTORY;
                            strcpy(recieved_transaction[i].filename, incoming->filename);
                            wroterecord = true;
                            valid_transaction[i] = true;
                        }
                    }
                    mtx.unlock();
                    if(!wroterecord){
                        std::cout<<"[RECV] Could not write record to transactions"<<std::endl;
                        exit(1);
                    }
                    system(workCMD);
                    ...
                    for(unsigned int i=0; i<100; i++){
                        if(!valid_transaction[i]){
                            recieved_transaction[i].fctype = incoming->fctype;
                            recieved_transaction[i].ftype = FILE_;
                            strcpy(recieved_transaction[i].filename, incoming->filename);
                            recieved_transaction[i].filesize = incoming->filesize;
                            wroterecord = true;
                            valid_transaction[i] = true;
                        }
                    }
                ...
                if(incoming->ftype == DIRECTORY){
                    ...
                    for(unsigned int i=0; i<100; i++){
                        if(!valid_transaction[i]){
                            recieved_transaction[i].fctype = DELETE;
                            recieved_transaction[i].ftype = DIRECTORY;
                            strcpy(recieved_transaction[i].filename, incoming->filename);
                            wroterecord = true;
                            valid_transaction[i] = true;
            ...
            while (event->len ) {
                if (event->mask & IN_CREATE) {
                    if (event->mask & IN_ISDIR){
                        bool transaction_recieved = false;
                        mtx.lock();
                        for(unsigned int i = 0; i<100; i++){
                            if(     valid_transaction[i]
                                    &&  recieved_transaction[i].ftype == DIRECTORY
                                    &&  recieved_transaction[i].fctype == CREATE
                                    &&  strcmp(recieved_transaction[i].filename, event->name)==0)
                            {
                                transaction_recieved = true;
                                valid_transaction[i] = false;
                                break;
                            }
                        }
                        mtx.unlock();
                        if(transaction_recieved){
                            std::cout<<"[FILEWATCHER] skip watching changes due to RECV."<<std::endl;
                            continue;
                        }
                ...
                if ( event->mask & IN_MODIFY) {
                    if (event->mask & IN_ISDIR){
                        ...
                        for(unsigned int i = 0; i<100; i++){
                            if(     valid_transaction[i]
                                    &&  recieved_transaction[i].ftype == DIRECTORY
                                    &&  recieved_transaction[i].fctype == MODIFY
                                    &&  strcmp(recieved_transaction[i].filename, event->name)==0)
                            {
                                transaction_recieved = true;
                                valid_transaction[i] = false;
                        ...
                        for(unsigned int i = 0; i<100; i++){
                            if(     valid_transaction[i]
                                    ...
                                    &&  recieved_transaction[i].fctype == CREATE
                                    &&  strcmp(recieved_transaction[i].filename, event->name)==0
                                    &&  recieved_transaction[i].filesize == outgoing->filesize)
                            {
                                transaction_recieved = true;
                                valid_transaction[i] = false;
                                break;
